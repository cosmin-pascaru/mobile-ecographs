...
class SA_MTSP_Solver(MTSP_Solver):
    ...
    class Solution:
        ...
        def compute_cost(self, distance_matrix, tour_limit, bonuses):
            ...
            for tour_st, tour_end in zip([0] + self.tour_ends[:-1], self.tour_ends):
                ...
                if circuit_cost > tour_limit:
                    circuit_cost *= SA_MTSP_Solver.TOO_LONG_TOUR_PENALTY
        ...
        def get_random_neighbour_and_cost(self, distance_matrix, tour_limit, bonuses):
            ...
            for neighbour_f in SA_MTSP_Solver.Solution.neighbour_functions:
                if p < neighbour_f[1]:
        ...
        def generate_random(nr_nodes, nr_tours):
            s = SA_MTSP_Solver.Solution(nr_nodes, nr_tours)
    ...
    def __init__(self, initial_temperature, cooling_rate):
        super(MTSP_Solver, self).__init__()
    ...
    def solve(self, distance_matrix, nr_tours, time_limit=float('inf'), tour_limit=float('inf')):
        ...
        current_solution = self.Solution.generate_random(self.nr_nodes, self.nr_tours)
...
SA_MTSP_Solver.Solution.neighbour_functions = [(SA_MTSP_Solver.Solution.swap2, 0.0),
                                               (SA_MTSP_Solver.Solution.swap2adj, 0.6),
                                               (SA_MTSP_Solver.Solution.swap3, 0.7),
                                               (SA_MTSP_Solver.Solution.change_lengths, 1.0)
                                               ]
